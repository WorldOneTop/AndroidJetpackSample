    ...
    companion object {
        ...
        private fun fillInDb(context: Context) {
            CoroutineScope(Dispatchers.IO).launch {
                getInstance(context).wordsDao().insert(
                    CHEESE_DATA.map { Words(id = 0, name = it) }
                )
            }
        }
    }
...
interface WordsDao {
    ...
    fun allSelect(): PagingSource<Int, Words>

    @Insert
    fun insert(word: List<Words>)

    @Insert
    fun insert(word: Words)

    @Delete
    fun delete(word: Words)
}
